...
$(document).ready(function () {
  ...
  //carica (se hai il consenso) la history tramite i cookies
  if (cookieEnabled)
    loadHistoryCookies();
  ...
  if (v) {
    ...
    {//prelievo codice comune + resto del codice
      getBirthPlaceCodeByName(BirthPlace).then(x => { //Devo leggere il file, visto che la lettura non ritorna subito il valore (perchè è funzione asincrona e non volevo sprecare ram per 
        ...
        {//codice calcolato, stampa a video + history (se attiva)
          if (cookieEnabled) {
...
function updateBirthPlaceSelector() {
  fetch(file)
    .then(response => {
      if (!response.ok) {
...
function getBirthPlaceCodeByName(name) {
  return fetch(file)//funzione asincrona, richiede che il codice aspetti il return
    .then(response => {
      if (!response.ok) {
        throw new Error('Impossibile aprire il file.');
      }
